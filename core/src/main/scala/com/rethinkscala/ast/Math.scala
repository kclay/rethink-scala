...
abstract case class All(left: Typed, right: Typed) extends BiCompareOperQuery {
  ...
  def termType: TermType = TermType.ALL
}
...
abstract case class Or(left: Typed, right: Typed) extends BiCompareOperQuery {
  ...
  def termType: TermType = TermType.ANY
}


object All {
  def apply(left: Binary, right: Binary):ProduceBinary = new All(left, right) with ProduceBinary

  def apply[T, C[_]](left: Typed, right: Sequence[T, C]):ProduceSeq[T,C] = new All(left, right) with ProduceSeq[T, C]

  def apply(left: Typed, right: Strings):ProduceString = new All(left, right) with ProduceString

  def apply(left: Typed, right: Numeric):ProduceNumeric = new All(left, right) with ProduceNumeric
}
